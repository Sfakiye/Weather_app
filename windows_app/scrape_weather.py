        ...
        if self.in_td:
            ...
            if data.replace(".", "").replace("-", "").isdigit():
                if self.col_index < 3:
                    ...
                    self.valid_row = True
                self.col_index += 1
    def fetch_weather_data(self, start_year, start_month, end_year=None):
        ...
        try:
            while True:
                ...
                for i, data_row in enumerate(self.all_data[:-3]):
                    try:
                ...
                if end_year is not None:
                    if year == end_year:
                        print(f"[INFO] Reached specified end year: {end_year}")
                        break
